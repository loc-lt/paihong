...
def _annotate_parts_count(queryset):
    return annotate_work_item_parts_progress(queryset)
    ...
    parts_qs = (
        ...
        .annotate(
            ...
            completed_steps=Count(
                "steps",
                filter=Q(steps__status=StepStatusEnum.DONE.value),
                distinct=True,
            ),
        )
...
class WorkItemViewSet(viewsets.ViewSet):
    ...
    def parts(self, request, pk=None):
        ...
        queryset = PartFilter(
            ...
            .annotate(
                _total_steps=Count("steps", distinct=True),
                _completed_steps=Count(
                    "steps",
                    filter=Q(steps__status=StepStatusEnum.DONE.value),
                    distinct=True,
                ),
        ...
        completed_parts = queryset.filter(
            _total_steps__gt=0,
            _total_steps=F("_completed_steps"),
        ).count()
        serializer = WorkItemPartsListSerializer(
            {
                "parts": queryset,
